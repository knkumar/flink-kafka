...
OUTPUT_TOPIC="bench-${WORKLOAD_ID}-flink-output"
GROUP_ID="stream-state-bench-flink-${WORKLOAD_ID}"
TRANSACTIONAL_ID_PREFIX="stream-state-bench-flink-${WORKLOAD_ID}"
...
if [[ "$WORKLOAD" == "stream_stream_join" ]]; then
  docker compose -f "$COMPOSE_FILE" exec -T kafka-1 /opt/kafka/bin/kafka-console-producer.sh \
    --bootstrap-server kafka-1:9092,kafka-2:9092,kafka-3:9092 \
...
docker compose -f "$COMPOSE_FILE" up --build --abort-on-container-exit --exit-code-from flink-identity flink-identity

docker compose -f "$COMPOSE_FILE" exec -T kafka-1 /opt/kafka/bin/kafka-console-consumer.sh \
  --bootstrap-server kafka-1:9092,kafka-2:9092,kafka-3:9092 \
  --topic "$OUTPUT_TOPIC" \
  --from-beginning \
