...
## High T1-T0 Latency Findings
The high `T1-T0` latency (approx 1s) is largely explained by the producer implementation in `kafka_latency_probe.py`. The probe delegates production to the `kafka-console-producer.sh` script via a subprocess pipe. While the Python script enforces accurate pacing (`time.sleep` to target emission rate) and explicitly flushes its stdout pipe to the subprocess, the Java-based console producer reads from standard input (`System.in`). Standard input streams in Java typically employ block buffering when not attached to a TTY. This means that records accumulate in the Java process's input buffer (e.g., 8KB) and are not handed to the underlying Kafka Producer—and thus not subject to `linger.ms` or `batch.size` constraints—until the buffer fills or the stream is closed. Consequently, at lower event rates, records experience an arbitrary buffering delay on the producer host before they are even instantiated as Kafka Producer records, inflating `T1-T0` independent of Kafka's actual network or append latency.
