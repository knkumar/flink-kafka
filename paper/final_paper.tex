...
sensitivity, and fault-injection outcomes on one shared host. Fifty
independent correctness trials, five per engine/workload combination,
matched their expected output with zero missing, unexpected, or
duplicate records. Under 100 events/sec load for 30 minutes, the
combined 	exttt{T2-T1} hop ranges from 4-6 milliseconds for W1-W2 to
4.3-6.2 seconds for W3-W4; it includes both event-time waiting and
engine work, because the harness does not yet record semantic
eligibility 	exttt{T\_e}. A sustained W3 tuning study contains three
valid trials for each engine and interval, each with 6,000 inputs and
997 matched outputs. Fault injection shows that several Kafka Streams
stateful trials did not finish within the observation window while the
corresponding Flink trials completed, but the artifact does not directly
measure recovery milestones or prove permanent data loss.
\end{abstract}
...
This paper does not estimate a saturation boundary, total-system cost
distribution, or recovery time. RQ1 has five independent correctness
trials for each engine/workload combination and a fixed-rate latency
picture for W1-W4. RQ2 reports container samples, not normalized
infrastructure cost. RQ3 reports post-injection output observations and
final verification within a bounded observation window.
...
\texttt{T1-T0}, \texttt{T2-T1}, and \texttt{T3-T2}. For W3 and W4,
\texttt{T2-T1} combines semantic event-time waiting, triggering, and
engine computation. It is not an engine-only processing metric because
...
producer batching from broker work. The combined \texttt{T2-T1} hop is
...
unexpected, or duplicate records. The median per-run p99 \texttt{T2-T1}
...
\texttt{T2-T1} remains near 4.25 seconds and \texttt{T3-T2} rises from
732.1 to 8,702.2 ms. These observations are compatible with different
timing paths, but \texttt{T2-T1} still includes semantic waiting and the
...
Table 5.6 presents median per-run p99 \texttt{T2-T1} values across the
...
expected outputs because the recreated container joined a new
...
hop in these runs.} Section 5.3 shows \texttt{T2-T1} rising from
...
\texttt{T2-T1} rises from 6,085 to 23,155 ms when
...
lacks a semantic eligibility timestamp. \texttt{T2-T1} is therefore a
...
verification at every rate. The paper is limited to fixed-load
observations.
...
the measured combined \texttt{T2-T1} hop is multi-second for W3 and W4
